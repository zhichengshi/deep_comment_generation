...
def get_batch(dataset, idx, bs, comment_pad_index):
    ...
    pad_trg_seq = [[comment_pad_index] *
                   (max_len - len(com_len)) for com_len in trg_lens]
...
def train(model, dataset, trg_pad_index, batch_size, optimizer, criterion, clip):
    ...
    while i < len(dataset):
        ...
        output = output[:,1:,:].view(-1,output_dim) # negelect the first token <sos>
...
if __name__ == "__mian__":
    ...
    identifier_embeddings = np.zeros((identifier_word2vec.syn0.shape[0] + 1,
                                      identifier_word2vec.syn0.shape[1]),
                                     dtype="float32")
    identifier_embeddings[:identifier_word2vec.syn0.
                          shape[0]] = identifier_word2vec.syn0

    token_embeddings = token_word2vec.wv.syn0

    device = "gpu"
    TREE_NODE_DIM = 256
    ENC_HID_DIM = 256
    IDENTIFIER_VOC_SIZE = 100000
    ENCODE_DIM = 128
    BATCH_SIZE = 64
    ...
    encoder = Encoder(TREE_NODE_DIM, ENC_HID_DIM, IDENTIFIER_VOC_SIZE,
                      ENCODE_DIM, BATCH_SIZE, True, identifier_embeddings)

    attn = Attention(ENCODE_DIM, DEC_HID_DIM)
    decoder = Decoder(TRG_EMBEDDING_SIZE, ENCODE_DIM, DEC_HID_DIM,
                      TRG_VOC_SIZE, OUTPUT_DROPOUT, token_embeddings, attn)
